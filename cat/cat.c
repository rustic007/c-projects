...
static void print_char_verbose(unsigned char ch, const Options *opts) {
  if (ch == '\r') {
    printf("^M");
  } else if (ch < 32 && !opts->show_end_lines) {
    printf("^%c", ch + 64);
  } else if (ch == 127) {
    printf("^?");
  } else if (ch >= 128) {
...
static void print_char(unsigned char ch, const Options *opts) {
  if (opts->implies_v) {
    print_char_verbose(ch, opts);
  } else if (opts->show_tab && ch == '\t') {
    printf("^I");
  } else {
    putchar(ch);
  }
...
Options parse_args(int argc, char *argv[]) {
  Options opts = {false, false, false, false, false, false, false, NULL};
  int opt;
  while ((opt = getopt(argc, argv, "nbeEtTsv")) != -1) {
    switch (opt) {
    case 'n':
    ...
    for (ssize_t i = 0; i < bytes_read; i++) {
      ...
      if (ch == '\n') {
        ...
        for (size_t j = 0; j < line_len; j++) {
          print_char((unsigned char)line[j], opts);
        }
...
int main(int argc, char *argv[]) {
  if (argc < 2) {
    fprintf(stderr, "Usage: %s -flags <filename>", argv[0]);
    return 1;
  }
  ...
  if (opts.error) {
    fprintf(stderr, "There is no such flag");
    return 1;
  }
